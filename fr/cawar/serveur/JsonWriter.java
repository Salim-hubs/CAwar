...
public class JsonWriter {
    ...
    private List<BulletDisplay> obstacles; // Liste des obstacles (on les traite comme des balles)
    private String type;
    ...
    public static String initPacket(int id, String map) {
        // Ecrit au format type = init, id = id, map = map
        return String.format("{\"type\": \"init\", \"id\": %d, \"map\": %s}", id, map);
    }
    ...
    public static ArrayList<KeyTimePair> jsonToKeyTimePairList(String json) {
        try {
            Gson gson = new Gson();
            Type listType = new TypeToken<ArrayList<KeyTimePair>>() {}.getType();
            return gson.fromJson(json, listType);
        } catch (Exception e) {
            System.err.println("❌ Erreur lors du parsing JSON : " + e.getMessage());
            e.printStackTrace();
            return new ArrayList<>(); // ou null si tu veux stopper le traitement
        }
    }
    ...
    public static void main(String[] args) {
        ...
        // Exemple de paquet init
        System.err.println("_______________________________________");
        String initPacket = jsonWriter.initPacket(1, obstacleJson);
